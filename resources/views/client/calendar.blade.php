<x-app-layout>
    ...
    <div class="py-8 min-h-screen">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- Flash messages --}}
            @if(session('success'))
                <div class="mb-4 flex items-center gap-3 px-5 py-4 bg-white border border-green-100 rounded-2xl shadow-sm">
                    <div class="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 bg-green-50">
            ...
            @if(session('error'))
                <div class="mb-4 flex items-center gap-3 px-5 py-4 bg-white border border-red-100 rounded-2xl shadow-sm">
            ...
            <div
                x-data="clientCalendar({{ $appointmentsJson }})"
                x-init="init()"
                class="space-y-4"
            >
                {{-- Controles --}}
                                ...
                                <template x-for="appt in appointmentsForDay(day.date)" :key="appt.id">
                                    <div :class="isPastAppointment(appt) ? 'opacity-65' : ''"
                                        ...
                                        <div :class="appointmentBorderClass(appt)"
                                            ...
                                            <p class="text-xs text-purple-400" x-text="appt.professional"></p>
                                        </div>
                ...
                <template x-teleport="body">
                    <div x-show="dayModalOpen"
                         ...
                         style="display: none;">
                        <div class="absolute inset-0" @click="closeModal()" style="background-color: rgba(146, 64, 204, 0.18);"></div>
                                    ...
                                    <template x-for="appt in appointmentsForDay(selectedDay)" :key="appt.id">
                                        <div :class="isPastAppointment(appt) ? 'opacity-65' : ''"
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <template x-if="appt.price">
                                                    <p class="text-xs text-purple-400 mt-0.5" x-text="'R$ ' + appt.price"></p>
                                                </template>
                                            </div>
                                            <div class="flex flex-col items-end gap-2">
                                                ...
                                                {{-- Botão cancelar --}}
                                                <template x-if="canCancel(appt)">
                                                    <form :action="'/appointments/' + appt.id + '/cancel'" method="POST">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit"
                                                                @click.prevent="confirmCancel($event)"
                                                                class="text-[10px] font-semibold text-red-400 hover:text-red-600 border border-red-200 hover:border-red-400 px-2 py-0.5 rounded-lg transition duration-150">
                                                            Cancelar
                                                        </button>
                                                    </form>
                                                </template>
    ...
    <script>
        function clientCalendar(appointments) {
            ...
            return {
                ...
                closeModal() {
                    ...
                    setTimeout(() => { if (!this.dayModalOpen) this.selectedDay = null; }, 110);
                },
                ...
                canCancel(appt) {
                    ...
                    return !this.isPastAppointment(appt);
                },
                ...
                appointmentTagClass(appt) {
                    if (appt.status === 'cancelled') return 'bg-red-100 text-red-700 border border-red-200';
                    if (appt.status === 'completed') return 'bg-purple-100 text-purple-700 border border-purple-200';
                    if (this.isPastAppointment(appt)) return 'bg-transparent text-purple-500 border border-purple-200';
                    return appt.status === 'confirmed' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700';
                },

                appointmentBorderClass(appt) {
                    if (appt.status === 'cancelled') return 'border-red-300';
                    if (appt.status === 'completed') return 'border-purple-300';
                    if (this.isPastAppointment(appt)) return 'border-purple-200';
                    return appt.status === 'confirmed' ? 'border-emerald-400' : 'border-amber-400';
                },

                appointmentDotClass(appt) {
                    if (appt.status === 'cancelled') return 'bg-red-400';
                    if (appt.status === 'completed') return 'bg-purple-400';
                    if (this.isPastAppointment(appt)) return 'bg-purple-300';
