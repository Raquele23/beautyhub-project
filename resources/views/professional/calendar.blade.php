...
<!-- <x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
                {{-- Controles --}}
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-base font-bold text-gray-900 dark:text-white min-w-[160px] text-center"
                            x-text="title()"></h3>
                        <button @click="next()"
                                class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                            <svg class="w-4 h-4 text-gray-600 dark:text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button @click="goToday()"
                                class="text-xs font-semibold text-purple-600 hover:text-purple-700 dark:text-purple-400 ml-2">
                            Hoje
                        </button>
                    </div>

                    {{-- Toggle mensal/semanal --}}
                    <div class="flex rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 self-start sm:self-auto">
                        <button @click="view = 'month'"
                                :class="view === 'month' ? 'bg-purple-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'"
                                class="px-4 py-2 text-xs font-semibold transition">
                            Mensal
                        </button>
                        <button @click="view = 'week'"
                                :class="view === 'week' ? 'bg-purple-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'"
                                class="px-4 py-2 text-xs font-semibold transition border-l border-gray-200 dark:border-gray-700">
                            Semanal
                        </button>
                    </div>
                </div>
                ...
                <div x-show="view === 'month'"
                    ...
                    <div class="grid grid-cols-7 border-b border-gray-100 dark:border-gray-700">
                        <template x-for="day in ['Dom','Seg','Ter','Qua','Qui','Sex','Sáb']">
                            <div class="py-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-400"
                                 x-text="day"></div>
                        </template>
                    </div>
                    ...
                    <div class="grid grid-cols-7">
                        <template x-for="(cell, idx) in monthCells()" :key="idx">
                            <div
                                @click="cell.date && selectDay(cell.date)"
                                :class="{
                                    'bg-gray-50 dark:bg-gray-900/20': !cell.date,
                                    'cursor-pointer hover:bg-purple-50 dark:hover:bg-purple-900/10': cell.date,
                                    'ring-2 ring-inset ring-purple-500': cell.date && isToday(cell.date),
                                }"
                                class="min-h-[80px] p-2 border-b border-r border-gray-100 dark:border-gray-700 relative">

                                <span x-show="cell.date"
                                      :class="isToday(cell.date) ? 'bg-purple-600 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold' : 'text-xs text-gray-500 dark:text-gray-400'"
                                      class="inline-block mb-1"
                                      x-text="cell.day"></span>

                                <template x-for="appt in appointmentsForDay(cell.date)" :key="appt.id">
                                    <div :class="appt.status === 'confirmed' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300'"
                                         class="text-[10px] font-medium px-1.5 py-0.5 rounded mb-0.5 truncate">
                                        <span x-text="appt.time + ' ' + appt.service"></span>
                                    </div>
                                </template>
                            </div>
                ...
                <div x-show="view === 'week'"
                    ...
                    <div class="grid grid-cols-7 border-b border-gray-100 dark:border-gray-700">
                        <template x-for="day in weekDays()" :key="day.date">
                            <div @click="selectDay(day.date)"
                                 class="py-3 text-center cursor-pointer hover:bg-purple-50 dark:hover:bg-purple-900/10 transition">
                                <p class="text-xs text-gray-500 dark:text-gray-400" x-text="day.label"></p>
                                <p :class="isToday(day.date) ? 'bg-purple-600 text-white rounded-full w-7 h-7 flex items-center justify-center mx-auto font-bold' : 'text-gray-800 dark:text-gray-200 font-semibold'"
                                   class="text-sm mt-1"
                                   x-text="day.num"></p>
                                <span x-show="appointmentsForDay(day.date).length > 0"
                                      class="inline-block w-1.5 h-1.5 bg-purple-500 rounded-full mt-1"></span>
                            </div>
                        </template>
                    </div>
                            ...
                            <div x-show="appointmentsForDay(day.date).length > 0" class="px-6 py-4">
                                ...
                                <template x-for="appt in appointmentsForDay(day.date)" :key="appt.id">
                                    <div class="flex items-center gap-3 mb-2">
                                        <span class="text-xs font-semibold text-gray-500 dark:text-gray-400 w-12 flex-shrink-0"
                                              x-text="appt.time"></span>
                                        <div :class="appt.status === 'confirmed' ? 'border-green-400' : 'border-yellow-400'"
                                             class="flex-1 border-l-2 pl-3">
                                            <p class="text-sm font-semibold text-gray-800 dark:text-gray-200"
                                               x-text="appt.service"></p>
                                            <p class="text-xs text-gray-500 dark:text-gray-400"
                                               x-text="appt.client"></p>
                                        </div>
                                        <span :class="appt.status === 'confirmed' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300'"
                ...
                {{-- Modal: lista do dia selecionado --}}
                <div x-show="selectedDay"
                     x-transition:enter="transition ease-out duration-150"
                     x-transition:enter-start="opacity-0"
                     x-transition:enter-end="opacity-100"
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md max-h-[80vh] overflow-y-auto">
                        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
                            <h4 class="font-bold text-gray-900 dark:text-white" x-text="selectedDayLabel()"></h4>
                            <button @click="selectedDay = null"
                        ...
                        <div class="p-4 space-y-3">
                            <template x-for="appt in appointmentsForDay(selectedDay)" :key="appt.id">
                                <div class="flex items-start gap-3 p-4 rounded-xl bg-gray-50 dark:bg-gray-700">
                                    <div :class="appt.status === 'confirmed' ? 'bg-green-400' : 'bg-yellow-400'"
                                         class="w-2 h-2 rounded-full mt-1.5 flex-shrink-0"></div>
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-semibold text-gray-800 dark:text-gray-200"
                                           x-text="appt.service"></p>
                                        <p class="text-xs text-gray-500 dark:text-gray-400"
                                           x-text="appt.client"></p>
                                        <p class="text-xs text-gray-400 mt-0.5" x-text="appt.time"></p>
                                    </div>
                                    <span :class="appt.status === 'confirmed' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300'"
                                          class="text-xs font-semibold px-2 py-0.5 rounded-full flex-shrink-0"
                                          x-text="appt.status === 'confirmed' ? 'Confirmado' : 'Pendente'"></span>
        ...
        function calendar(appointments) {
            ...
            return {
                ...
                title() {
                    ...
                    const week = this.weekDays();
                    return week[0].num + ' – ' + week[6].num + ' ' + months[this.current.getMonth()];
                },
