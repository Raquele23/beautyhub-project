        ...
        <form action="{{ route('services.update', $service) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-purple-100 shadow-sm p-6">
                ...
                <div class="grid grid-cols-[minmax(0,1fr)_auto] gap-4 items-start">
                        ...
                        <input type="file" name="image" id="service_image_input" accept="image/jpeg,image/png,image/webp" class="sr-only"
                               ...
                               data-original-src="{{ $service->image ? Storage::url($service->image) : '' }}">
                    </label>
                    ...
                    <div class="w-20 flex-shrink-0">
                        ...
                        <div id="service_preview_container" class="relative w-20 h-20 rounded-xl overflow-hidden border border-purple-100" style="background-color: #EDE4F8;">
                            <img id="service_preview_img" src="{{ $service->image ? Storage::url($service->image) : '' }}" class="w-full h-full object-cover {{ $service->image ? '' : 'hidden' }}" alt="Prévia da imagem do serviço">
                            <div id="service_preview_placeholder" class="absolute inset-0 flex items-center justify-center text-[10px] font-semibold text-purple-300 text-center px-2 {{ $service->image ? 'hidden' : '' }}">
                                Sem foto
                            </div>
                        </div>
                    </div>
                ...
                <div class="mt-3 flex items-center gap-3">
                    <button type="button"
                            ...
                            class="text-xs font-semibold text-purple-600 hover:text-purple-800 transition-colors {{ $service->image ? '' : 'hidden' }}">
                        Recortar novamente
                    </button>
                    <button type="button" id="service_remove_preview"
                            class="text-xs font-semibold text-red-500 hover:text-red-700 transition-colors {{ $service->image ? '' : 'hidden' }}">
                        Remover foto
                    </button>
                </div>
